...
import { Colors, Spacing, Typography } from '@/shared/constants/theme.constants';
...
const StepItem = ({
  ...
  const numberAnimatedStyle = useAnimatedStyle(() => {
    return {
      ...
      transform: [{ scale: withTiming(isCompleted ? 0.5 : 1, { duration: 200 }) }],
    };
  });
  ...
  return (
    <View style={[styles.stepContainer, { flex: isLastStep ? 0 : 1 }]}>
      <View style={styles.stepWrapper}>
        <Animated.View style={[styles.circle, dynamicStyles.circle, circleAnimatedStyle]}>
          <Animated.View
            style={[StyleSheet.absoluteFill, styles.centerContent, numberAnimatedStyle]}
          >
            <Text
              style={[
          ...
          <Animated.View
            style={[StyleSheet.absoluteFill, styles.centerContent, checkmarkAnimatedStyle]}
          >
            <Ionicons name="checkmark-sharp" size={size * 0.7} color={Colors.light.textInverse} />
          </Animated.View>
        </Animated.View>
      </View>
...
const Stepper = ({ steps = 3, currentStep, size = 28, containerStyle }: StepperProps) => {
